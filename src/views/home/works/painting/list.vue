<template>
  <div>
    ...
    <div style="height: 800px; width: 100%; overflow-x: hidden; overflow-y: scroll; padding-bottom: 20px; margin-top: 20px;">
      <vueWaterfallEasy ref="waterfall" :max-cols="maxCols" :imgs-arr="items" @scrollReachBottom="getPaintingList" @click="showDetail">
    ...
    getPaintingList() {
      list(this.pageConfig).then(response => {
        ...
        response.data.items.forEach(element => {
          this.items.push({ 'culturesId': element.culturesId, 'info': element.title, 'src': element.image })
        })
        this.pageConfig.page++
      })
    },
    showDetail(event, { index, value }) {
      // 阻止a标签跳转
      event.preventDefault()
      // 只有当点击到图片时才进行操作
      if (event.target.tagName.toLowerCase() === 'img') {
        this.$router.push(`/paintingDetail/` + value.culturesId)
      }
    }
  }
}
